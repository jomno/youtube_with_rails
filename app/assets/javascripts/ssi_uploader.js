...
(function (root, factory) {
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        var $chooseBtn = $('' +
            ...
            '</span>').append(element);
        var $uploadBtn = $('<button id="ssi-uploadBtn2" class="ssi-button success ssi-hidden" >' +
        ...
        $('#ssi-uploadBtn').click(function (e){
            e.preventDefault();
...
*/
            ...
            if ((file.size * Math.pow(10, -6)).toFixed(2) > this.options.maxFileSize) {//if file size is more than we ask
                if (length > filesLength) {
                    filesLength++;
                } else {
                    toUploadLength--;
    ...
    Ssi_upload.prototype.uploadFiles = function (data) {// upload the pending files
        if (this.pending > 0) {
            ...
            var $uploadBtn = this.$element.find('#ssi-uploadBtn'),f
                $clearBtn = this.$element.find('#ssi-clearBtn');
            $uploadBtn.prop("disabled", true);
            var thisS = this,
            ...
            $.each(data, function (key, value) {// append all extra data
        ...
        function ajaxLoopRequest(formDatas, ii) {
